// Ported verbatim from client/src/features/rank-predictor/utils/collegeTiers.js.
//
// PRESENTATION ONLY. This does NOT change prediction, rank calculation, or
// cutoff data. It simply re-groups the EXISTING college_cutoffs.json bands
// for the student's category into Dream / Target / Safe buckets by comparing
// the already-computed predicted AIR range against each college's existing
// [low, high] band. No new data, no model, no statistics.
import collegeCutoffs from "@/data/rank-predictor/college_cutoffs.json";

export type CutoffCollege = { college: string; low: number; high: number };
export type RankRange = { rankLow: number; rankHigh: number } | null;
export type TieredCollege = CutoffCollege & { tier: string; chance: number };

/** Rule-based "chance" purely from where the rank sits in/around the band. */
function classify(college: CutoffCollege, point: number): { tier: string; chance: number } {
 ...
 */
export function getTieredColleges(category: string, rr: RankRange) {
  const list = (collegeCutoffs as Record<string, CutoffCollege[]>)[category] || [];
  if (!rr) return { dream: [] as TieredCollege[], target: [] as TieredCollege[], safe: [] as TieredCollege[], reach: [] as TieredCollege[], point: null };

  // Point estimate = midpoint of the EXISTING predicted range (no new prediction).
  const point = Math.round((rr.rankLow + rr.rankHigh) / 2);

  const classified: TieredCollege[] = list.map((c) => ({ ...c, ...classify(c, point) }));
 ...
 */
export function chanceFor(college: CutoffCollege | null | undefined, rr: RankRange): number | null {
