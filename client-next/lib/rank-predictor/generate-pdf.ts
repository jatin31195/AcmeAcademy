// Ported verbatim from client/src/features/rank-predictor/utils/generatePdf.js.
// jsPDF programmatic PDF, no html2canvas. Colors: hex strings only (jsPDF
// accepts them directly). Presentation/branding only — the prediction
// values (rr, colleges) are passed in from the caller and are NOT
// recomputed here.
import { jsPDF } from "jspdf";
import { ACME } from "@/lib/rank-predictor/constants";

type RankRange = { rankLow: number; rankHigh: number } | null;
type ReportFormData = { name: string; regNo: string; category: string; marks: number | string; city: string; state: string };
type Colleges = { top: string; fallback: string };
...
export async function generatePdf({
  ...
  const text = (str: string | number, x: number, fy: number, opts?: Record<string, unknown>) => doc.text(String(str), x, fy, opts || {});

  /* ═══════════════════════════════════════════════════
     1. TOP GRADIENT BAR
  ═══════════════════════════════════════════════════ */
  fill("#2563eb");
  doc.rect(0, 0, PW / 2, 7, "F");
  fill("#4338ca");
  doc.rect(PW / 2, 0, PW / 2, 7, "F");
  y = 7;
  ...
  if (logoDataUrl) {
    try {
      doc.addImage(logoDataUrl, "PNG", margin, y, 52, 52);
    } catch {}
  }
  ...
  // Marks box (blue)
  fill("#2563eb");
  doc.roundedRect(margin, y, bw, bh, 8, 8, "F");
  font("bold", 8);
  txt("#bfdbfe");
  ...
  fill("#4338ca");
  doc.roundedRect(rx2, y, bw, bh, 8, 8, "F");
  font("bold", 8);
  txt("#c7d2fe");
  ...
  fill("#059669");
  doc.circle(margin + 24, y + 30, 13, "F");
  font("bold", 13);
  txt("#ffffff");
  ...
  if (colleges.fallback && colleges.fallback !== "None" && colleges.fallback !== "Not Available") {
    ...
    fill("#94a3b8");
    doc.circle(margin + 24, y + 26, 11, "F");
    font("bold", 11);
    txt("#ffffff");
