...
// Ported from client/src/pages/PYQ.jsx. Search input + exam/year <Select>s
// are genuinely wired to client state (unlike FreeCourses' dead filter
// state), so this stays a Client Component receiving the server-fetched
// list as a prop, filtering it exactly as the original did.
export function PYQListClient({ initialPyqs }: { initialPyqs: Pyq[] }) {
  ...
  const filteredPYQ = initialPyqs.filter((pyq) => {
    const matchesSearch =
      pyq.title.toLowerCase().includes(searchTerm.toLowerCase()) || pyq.exam.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 via-gray-100 to-gray-200">
      <section className="relative py-16 sm:py-34 text-center overflow-hidden hero-gradient">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            <span className="bg-gradient-to-r from-pink-400 via-purple-300 to-indigo-300 text-transparent bg-clip-text">Previous</span>{" "}
      ...
      <section className="py-16">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPYQ.map((pyq) => (
            <Card
              key={pyq._id}
              onClick={() => router.push(`/pyq/${pyq._id}`)}
            ...
            >
              ...
              <CardContent>
                ...
                <div className="text-sm text-muted-foreground">
                  <span className="font-medium">Questions:</span> {pyq.questions} | <span className="font-medium">Size:</span> {pyq.fileSize}
                </div>
              </CardContent>
            </Card>
          ))}
