...
import { useCountUp } from "@/lib/rank-predictor/use-count-up";

// Ported verbatim from client/src/features/rank-predictor/components/ResultHero.jsx.
// ResultHero — the emotional payoff. Animated count-up of the predicted
// rank, celebration banner + confetti, and prominent (but professional) ACME
// branding. No prediction logic here — it only DISPLAYS the `rr` / `rankStr`
// produced by the existing engine.
type RankRange = { rankLow: number; rankHigh: number } | null;
type ReportData = { name?: string; marks?: number | string; category?: string };
      ...
      <div style={{ padding: "28px 28px 30px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 22 }}>
          <div style={{ background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.25)", borderRadius: 18, padding: 10, backdropFilter: "blur(6px)" }}>
            <Image src="/logo.png" alt="ACME Academy" width={842} height={711} style={{ height: 54, width: 54, objectFit: "contain" }} />
          </div>
          <div>
            <h1 style={{ margin: 0, fontSize: "clamp(18px,3.4vw,24px)", fontWeight: 900, lineHeight: 1.15, letterSpacing: "-0.4px" }}>{ACME.productName}</h1>
            <p style={{ margin: "4px 0 0", fontSize: 13.5, color: "rgba(219,234,254,0.92)" }}>{ACME.tagline}</p>
          </div>
          <span
            style={{
              marginLeft: "auto",
        ...
        >
          ...
          <div style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.18)", borderRadius: 16, padding: "18px 20px" }}>
            <p style={{ margin: 0, fontSize: 11, fontWeight: 700, color: "#bfdbfe", textTransform: "uppercase", letterSpacing: "0.07em" }}>Your NIMCET Marks</p>
          ...
          <div style={{ background: "rgba(255,255,255,0.16)", border: "1px solid rgba(255,255,255,0.28)", borderRadius: 16, padding: "18px 20px", position: "relative" }}>
            <p style={{ margin: 0, fontSize: 11, fontWeight: 700, color: "#e0e7ff", textTransform: "uppercase", letterSpacing: "0.07em" }}>Predicted All India Rank</p>
            ...
            <p style={{ margin: "8px 0 0", fontSize: 12, color: "#e0e7ff" }}>Expected AIR range · {ACME.methodologyLine}</p>
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 12, marginTop: 22 }}>
          ...
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 12 }}>
            <a href={ACME.website} target="_blank" rel="noopener noreferrer" style={{ fontSize: 13, fontWeight: 700, color: "#fff", textDecoration: "none", borderBottom: "1px solid rgba(255,255,255,0.5)" }}>
              {ACME.websiteLabel}
            </a>
            <button
