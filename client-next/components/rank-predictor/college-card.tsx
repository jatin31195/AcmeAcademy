...
import { getCollegeMeta } from "@/lib/rank-predictor/college-meta";
import { ACME } from "@/lib/rank-predictor/constants";

// Ported verbatim from client/src/features/rank-predictor/components/CollegeCard.jsx.
const TIER_STYLE: Record<string, { color: string; bg: string; border: string; grad: string; label: string }> = {
...
type CollegeCardData = { college: string; low: number; high: number; tier: string; chance: number | null };
 ...
 * Metadata (image/logo/location/seats/fees/packages/website) comes from college-meta.ts.
 */
const CollegeCard = ({ college, category, badge }: { college: CollegeCardData; category: string; badge?: string }) => {
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "relative", height: 150, background: ts.grad, overflow: "hidden" }}>
        {showImage && (
          // eslint-disable-next-line @next/next/no-img-element -- variable-count cards with unknown remote/local image outcomes; onError fallback needs a plain <img>, not next/image's build-time optimization
          <img
            src={meta.image}
            alt={`${name} campus`}
            loading="lazy"
        ...
        <div style={{ position: "absolute", left: 14, bottom: -20, height: 46, width: 46, borderRadius: 12, background: "#fff", border: "1px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 12px rgba(0,0,0,0.14)", overflow: "hidden" }}>
          {meta.logo && !logoError ? (
            // eslint-disable-next-line @next/next/no-img-element -- onError fallback to initials chip needs a plain <img>
      ...
      <div style={{ padding: "26px 16px 16px", flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        {meta.shortName && meta.shortName !== name && <span style={{ fontSize: 11.5, color: "#94a3b8", fontWeight: 600 }}>{meta.shortName}</span>}
          ...
          {meta.officialWebsite ? (
            <a
              ...
              style={{
                ...
                fontWeight: 800,
                textDecoration: "none",
                boxShadow: hover ? "0 8px 18px rgba(37,99,235,0.28)" : "none",
            ...
            <div style={{ textAlign: "center", padding: "11px 14px", borderRadius: 11, background: "#f1f5f9", color: "#94a3b8", fontSize: 13, fontWeight: 700 }}>Details coming soon</div>
          )}
...
function Stat({ label, value, ts }: { label: string; value?: string; ts: { color: string } }) {
  return (
    <div style={{ background: "#f8fafc", border: "1px solid #eef2f7", borderRadius: 10, padding: "8px 6px", textAlign: "center" }}>
      <p style={{ margin: 0, fontSize: 12.5, fontWeight: 800, color: value ? "#0f172a" : "#cbd5e1", lineHeight: 1.2 }}>{value || "—"}</p>
      <p style={{ margin: "2px 0 0", fontSize: 9.5, color: ts.color, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.04em" }}>{label}</p>
    </div>
  );
}
