...
// Ported verbatim from client/src/features/rank-predictor/components/ShareResult.jsx.
// ShareResult — generates a 1080×1080 square share card for WhatsApp /
// Instagram using the native Canvas API (same reliable approach as the CUET
// score card, NOT html2canvas). Presentation only.
const ShareResult = ({
  data,
  rankStr,
  eligibleCount = 0,
  ...
  const buildCanvas = (): Promise<HTMLCanvasElement> =>
    new Promise((resolve) => {
      ...
      const fit = (text: string, max: number, start: number, min = 22) => {
        let s = start;
      ...
      const draw = (logoImg: HTMLImageElement | null) => {
        ...
        ctx.fillText(new Date().toLocaleDateString("en-IN", { day: "2-digit", month: "long", year: "numeric" }), 540, 1044);
        ctx.textAlign = "left";
  ...
  const run = async (action: string) => {
    ...
    try {
      ...
      const file = new File([blob as BlobPart], fileName, { type: "image/png" });
  ...
  const btn = (label: string, action: string, bg: string) => (
    <button
      onClick={() => run(action)}
      disabled={!!busy}
      style={{
