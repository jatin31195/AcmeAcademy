...
import marksToRank from "@/data/rank-predictor/marks_to_rank.json";
import collegeCutoffs from "@/data/rank-predictor/college_cutoffs.json";
import { generatePdf } from "@/lib/rank-predictor/generate-pdf";
import { chanceFor } from "@/lib/rank-predictor/college-tiers";
import { reportNumber } from "@/lib/rank-predictor/branding";
import { RP_BASE, ACME, RP_REPORT_DATA_KEY } from "@/lib/rank-predictor/constants";

import ReportHeader from "@/components/rank-predictor/report-header";
import ResultHero from "@/components/rank-predictor/result-hero";
import CollegeCard from "@/components/rank-predictor/college-card";
import ShareResult from "@/components/rank-predictor/share-result";

import photo1 from "@/assets/images/rank-predictor/topper1.jpg";
import photo2 from "@/assets/images/rank-predictor/topper2.jpg";
import photo3 from "@/assets/images/rank-predictor/batch1.png";
import photo4 from "@/assets/images/rank-predictor/batch2.png";

// Course-promo images for the Advertisement Section (added from testing build).
import ad1 from "@/assets/images/rank-predictor/ad_course1.png";
import ad2 from "@/assets/images/rank-predictor/ad_course2.png";
import ad3 from "@/assets/images/rank-predictor/ad_course3.png";

// Ported from client/src/features/rank-predictor/pages/ReportPage.jsx.
//
// The original read its data from `location.state` (set by FormPage's
// `navigate(path, { state: payload })`) — Next.js Client Components on
// separate routes have no equivalent, so this reads the same payload back
// from sessionStorage (RP_REPORT_DATA_KEY), written by FormPage right
// before navigating here. See constants.ts for the full rationale. The
// "redirect to RP_BASE if no data" guard is preserved exactly.
type FormPayload = {
  name: string;
  phone: string;
  marks: number;
...
const ReportPage = () => {
  ...
  useEffect(() => {
    const raw = sessionStorage.getItem(RP_REPORT_DATA_KEY);
    if (raw) {
      try {
        // eslint-disable-next-line react-hooks/set-state-in-effect -- sessionStorage read replacing router-state, ported behavior
  ...
  useEffect(() => {
    if (checked && !data) router.push(RP_BASE);
  ...
  /* ─────────────────────────────────────────────────────────────
     ...
     ───────────────────────────────────────────────────────────── */
  const rankRecord = (marksToRank as { min_marks: number; max_marks: number; rank_low: number; rank_high: number }[]).find(
    (r) => data.marks >= r.min_marks && data.marks <= r.max_marks
  );
  ...
  const catList = [...((collegeCutoffs as Record<string, { college: string; low: number; high: number }[]>)[data.category] || [])].sort((a, b) => a.high - b.high);
  ...
  const previewColleges = [topEligible, ...fallbacks]
  ...
  const handleDownloadPDF = async () => {
    setPdfBusy(true);
    try {
      await generatePdf({ data, rr, colleges, logoSrc: "/logo.png", reportNo: reportNumber(data) });
    } catch (e) {
  ...
  const sectionGap: React.CSSProperties = { display: "flex", flexDirection: "column", gap: 22 };
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", background: "#f1f5f9", padding: "20px 16px 48px" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", ...sectionGap }}>
        ...
        <div style={card()}>
          <h2 style={{ margin: "0 0 4px", fontSize: 18, fontWeight: 800, color: "#0f172a" }}>College Recommendations</h2>
          ...
          <h3 style={{ margin: "0 0 2px", fontSize: 15, fontWeight: 800, color: "#0f172a" }}>Top Eligible College</h3>
          <p style={{ margin: "0 0 14px", fontSize: 12.5, color: "#64748b" }}>The best college matched to your predicted rank</p>
          {topEligibleCard ? (
            <div style={{ maxWidth: 360 }}>
              <CollegeCard college={topEligibleCard} category={data.category} badge="Top Eligible" />
            </div>
          ) : (
            <div style={emptyBox}>Your predicted rank is outside the listed {data.category} cutoffs. Aim higher with ACME&apos;s NIMCET courses.</div>
          )}

          {/* Fallback Option */}
          <h3 style={{ margin: "26px 0 2px", fontSize: 15, fontWeight: 800, color: "#0f172a" }}>Fallback Option</h3>
          <p style={{ margin: "0 0 14px", fontSize: 12.5, color: "#64748b" }}>Safer backups you can confidently target, with your admission chances</p>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1fr)", gap: 18 }} className="rp-profile-grid">
            ...
            <div>
              ...
              <p style={{ margin: "10px 0 0", fontSize: 11.5, color: "#94a3b8" }}>
                {ACME.methodologyLine} · {ACME.poweredBy}
              </p>
            </div>
            ...
            <div style={{ borderRadius: 14, overflow: "hidden", border: "1px solid #e2e8f0", background: "#0f172a" }}>
              <div style={{ padding: "9px 14px", background: "#1e293b", borderBottom: "1px solid #334155", textAlign: "center" }}>
                <p style={{ margin: 0, fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.08em" }}>ACME Top Achievers</p>
              </div>
              <div style={{ position: "relative", height: 190, overflow: "hidden" }}>
                <div style={{ display: "flex", height: "100%", transform: `translateX(-${slide * 100}%)`, transition: "transform 0.6s ease" }}>
                  {photos.map((p) => (
                    <div key={p.id} style={{ width: "100%", height: "100%", flexShrink: 0, position: "relative" }}>
                      <a href={p.link} target="_blank" rel="noopener noreferrer" style={{ display: "block", width: "100%", height: "100%", position: "relative" }}>
                        <Image src={p.url} alt={p.alt} fill style={{ objectFit: "contain" }} />
                      </a>
                    </div>
                  ))}
                </div>
                <div style={{ position: "absolute", bottom: 10, left: 0, right: 0, display: "flex", justifyContent: "center", gap: 6 }}>
                  {photos.map((_, i) => (
                    <div
                      key={i}
                      onClick={() => setSlide(i)}
                      style={{ cursor: "pointer", height: 5, borderRadius: 3, transition: "width 0.3s, background 0.3s", width: i === slide ? 18 : 5, background: i === slide ? "#fff" : "rgba(255,255,255,0.3)" }}
                    />
                  ))}
                </div>
              </div>
        ...
        {/* 7. SHARE */}
        <div style={card()}>
          <ShareResult data={data} rr={rr} rankStr={rankStr} eligibleCount={eligibleCount} previewColleges={previewColleges} topCollege={topCollege} />
        </div>
        ...
        >
          <div style={{ textAlign: "center", marginBottom: "24px" }}>
            ...
            <h3 style={{ fontSize: "clamp(18px, 3vw, 24px)", fontWeight: "800", color: "#0f172a", margin: "0 0 8px" }}>Didn&apos;t make it to your dream college this year?</h3>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "20px" }}>
            ...
            <a href="https://acmea.courses.store/615002" target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none", display: "flex" }}>
              <div
                style={{ flex: 1, borderRadius: "16px", background: "#f1f5f9", border: "1px solid #e2e8f0", display: "flex", flexDirection: "column", overflow: "hidden", transition: "transform 0.2s, box-shadow 0.2s" }}
                onMouseEnter={(e) => {
              ...
              >
                <Image src={ad1} alt="Lakshya Dropper Batch" style={{ width: "100%", height: "auto", aspectRatio: "1", objectFit: "cover", borderBottom: "1px solid #e2e8f0" }} />
                <div style={{ padding: "20px", display: "flex", flexDirection: "column", flex: 1 }}>
                  ...
                  <h4 style={{ fontSize: "16px", fontWeight: "800", color: "#0f172a", margin: "0 0 8px", lineHeight: 1.4 }}>NIMCET 2027 Lakshya Dropper Batch | NIMCET-2027 &amp; CUET</h4>
            ...
            <a href="https://acmea.courses.store/380094" target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none", display: "flex" }}>
              ...
              >
                <Image src={ad2} alt="VOD Course" style={{ width: "100%", height: "auto", aspectRatio: "1", objectFit: "cover", borderBottom: "1px solid #e2e8f0" }} />
                <div style={{ padding: "20px", display: "flex", flexDirection: "column", flex: 1 }}>
                  <div style={{ display: "flex", gap: "8px", marginBottom: "10px", flexWrap: "wrap" }}>
                  ...
                  <h4 style={{ fontSize: "16px", fontWeight: "800", color: "#0f172a", margin: "0 0 8px", lineHeight: 1.4 }}>VOD COURSE | NIMCET 2027 | CUET 2027 (ASSC-ACME Self...)</h4>
                  <div style={{ flex: 1 }}></div>
                  <div style={{ marginTop: "16px", display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
                    <div>
            ...
            <a href="https://acmea.courses.store/290322" target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none", display: "flex" }}>
              ...
              >
                <Image src={ad3} alt="Test Series" style={{ width: "100%", height: "auto", aspectRatio: "1", objectFit: "cover", borderBottom: "1px solid #e2e8f0" }} />
                <div style={{ padding: "20px", display: "flex", flexDirection: "column", flex: 1 }}>
                  <div style={{ display: "flex", gap: "8px", marginBottom: "10px", flexWrap: "wrap" }}>
                  ...
                  <h4 style={{ fontSize: "16px", fontWeight: "800", color: "#0f172a", margin: "0 0 8px", lineHeight: 1.4 }}>
                    ACME Premium Test Series
                    <br />
                    <span style={{ fontSize: "13px", fontWeight: "600", color: "#94a3b8" }}>Target: NIMCET, CUET &amp; MAH-CET</span>
                  </h4>
                  <div style={{ flex: 1 }}></div>
                  <div style={{ marginTop: "16px", display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
                    <div>
          ...
          <div style={{ textAlign: "center", marginTop: "24px", paddingTop: "20px", borderTop: "1px solid #e2e8f0" }}>
            <a
              ...
              style={{ display: "inline-flex", alignItems: "center", gap: "8px", fontSize: "15px", fontWeight: "700", color: "#2563eb", textDecoration: "none", padding: "8px 16px", borderRadius: "8px", transition: "background 0.2s" }}
