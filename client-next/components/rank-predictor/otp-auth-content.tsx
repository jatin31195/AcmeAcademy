...
const OtpAuthPage = () => {
  ...
  useEffect(() => {
    if (db) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- db-ready check on mount, ported behavior
      setBackendReady(true);
      return;
    }
    const interval = setInterval(async () => {
      try {
        const res = await fetch(`${BACKEND_URL}/api/config/firebase`, { signal: AbortSignal.timeout(5000) });
        if (res.ok) {
  ...
  const verifyOtp = async () => {
    ...
    setIsLoading(true);
    try {
      // Verify is a POST with a JSON body (sessionId/otp no longer travel in
      // the URL where they could be logged or cached).
      ...
      // Surface the REAL reason for failure instead of blanket "Invalid OTP".
  ...
  return (
    <div style={{ display: "flex", flex: 1, overflow: "auto", transition: "background 0.3s", flexDirection: "column" }}>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "auto" }}>
        ...
        >
          {/* blobs */}
          {[
            { t: "-100px", l: "-100px", s: "350px", o: "0.15" },
            { b: "-80px", r: "-80px", s: "280px", o: "0.1" },
          ].map((b, i) => (
            <div
              key={i}
              style={{
          ...
          <div style={{ position: "relative", zIndex: 1, textAlign: "center", color: "#fff" }}>
            <div style={{ display: "flex", justifyContent: "center", marginBottom: "28px" }}>
              <div style={{ background: "rgba(255,255,255,0.15)", borderRadius: "24px", padding: "18px", border: "1px solid rgba(255,255,255,0.25)", backdropFilter: "blur(8px)" }}>
                <Image src="/logo.png" alt="ACME" width={842} height={711} style={{ height: "100px", width: "100px", objectFit: "contain" }} />
              </div>
            </div>
            ...
            <p style={{ fontSize: "15px", color: "rgba(219,234,254,0.9)", lineHeight: 1.7, margin: "0 0 32px" }}>
              ...
              <br />
              NIMCET rank prediction &amp; counselling.
            </p>
            {["AI-powered rank prediction", "College eligibility analysis", "Instant PDF report download"].map((f) => (
              <div key={f} style={{ display: "flex", alignItems: "center", gap: "10px", margin: "0 0 12px", fontSize: "14px", color: "rgba(219,234,254,0.9)", fontWeight: 500, textAlign: "left" }}>
        ...
        {/* ══ RIGHT PANEL — form ══ */}
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "48px 32px", backgroundColor: bg, overflowY: "auto", transition: "background-color 0.3s" }}>
          {/* Mobile logo */}
          <div className="flex lg:hidden" style={{ flexDirection: "column", alignItems: "center", marginBottom: "28px", textAlign: "center" }}>
            <div style={{ background: surface, borderRadius: "18px", padding: "10px", border: `1px solid ${border}`, marginBottom: "10px" }}>
              <Image src="/logo.png" alt="ACME" width={842} height={711} style={{ height: "56px", width: "56px", objectFit: "contain" }} />
            </div>
          ...
          <div style={{ width: "100%", maxWidth: "420px" }}>
            <div style={{ marginBottom: "32px" }}>
              <h1 style={{ fontSize: "32px", fontWeight: "900", color: txtMain, margin: "0 0 8px", letterSpacing: "-0.5px" }}>{step === "phone" ? "Get Started" : "Verify OTP"}</h1>
              <p style={{ fontSize: "15px", color: txtMuted, margin: 0 }}>
                {step === "phone" ? "Enter your registered mobile number to continue" : `OTP sent to +91 ${phone}`}
              </p>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
              {step === "phone" ? (
                <>
                  <div>
                    <label style={{ display: "block", fontSize: "14px", fontWeight: "600", color: txtMain, marginBottom: "8px" }}>Mobile Number</label>
                    <div
                      style={{ display: "flex", borderRadius: "12px", border: `2px solid ${border}`, overflow: "hidden", background: inputBg, transition: "border-color 0.2s" }}
                      onFocusCapture={(e) => (e.currentTarget.style.borderColor = "#2563eb")}
                      onBlurCapture={(e) => (e.currentTarget.style.borderColor = border)}
                    >
                      <span style={{ padding: "14px 16px", background: prefixBg, borderRight: `2px solid ${border}`, fontSize: "14px", fontWeight: "700", color: txtMuted, flexShrink: 0, userSelect: "none" }}>
                        +91
                      </span>
                      <input
                        type="tel"
                        ...
                        maxLength={10}
                        style={{ flex: 1, padding: "14px 16px", background: "transparent", border: "none", outline: "none", fontSize: "16px", fontWeight: "500", color: txtMain, fontFamily: "inherit" }}
                      />
                    </div>
                  </div>
                ...
                <>
                  <div>
                    <label style={{ display: "block", fontSize: "14px", fontWeight: "600", color: txtMain, marginBottom: "8px" }}>Enter OTP</label>
                    <input
                      type="text"
                      inputMode="numeric"
                      value={otp}
                  ...
                  <button
                    ...
                    style={{ background: "none", border: "none", cursor: "pointer", fontSize: "14px", fontWeight: "600", color: "#2563eb", padding: "4px", textAlign: "center" }}
                  >
                    ← Change phone number
                  </button>
                </>
              ...
              {error && (
                ...
                >
                  <svg width="18" height="18" style={{ flexShrink: 0, marginTop: "1px" }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
...
/* ── Shared CTA button ── */
const Btn = ({ onClick, loading, label }: { onClick: () => void; loading: boolean; label: string }) => (
  <button
    onClick={onClick}
    disabled={loading}
    style={{
