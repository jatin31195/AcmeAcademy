...
// Ported from client/src/pages/Contact.jsx. Same rationale as About's
// Client-body split: form state, fetch, toast, hash-scroll effect, and
// animated stat counters make nearly the whole page interactive, so the
// visual body is one Client Component; generateMetadata/JSON-LD stay in the
// Server page.tsx wrapper. `MapPin`, `BookOpen`, `Users2` were imported in
// the original but never used in its JSX — dropped here to keep lint clean
// (dead imports, not a behavior change; `MapPin` IS used lower in this file
// for icon usage checks below — kept only the icons actually referenced).
const quickContacts = [
  {
    icon: Phone,
    title: "Call Us",
...
const centers = [
  {
    name: "Kanpur Center",
    address: "Acme Academy (NIMCET MCA), Kanpur, Uttar Pradesh 208016",
    phone: "+91 9516001679",
    embedMap:
  ...
  {
    name: "Raipur Center",
    address: "ACME Academy, Near Fafadih Chowk, Raipur, Chhattisgarh 492001",
    phone: "+91 8109977628",
    embedMap:
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 via-white to-pink-50 text-gray-800">
      ...
      <section className="relative py-16 sm:py-28 text-center overflow-hidden hero-gradient">
        ...
        >
          <h1 className="text-[clamp(2.5rem,6vw,4rem)] font-extrabold mb-4 text-white drop-shadow-lg">Contact ACME Academy</h1>
          <p className="text-gray-800 text-lg sm:text-xl  max-w-3xl mx-auto">
            Have a question, need guidance, or want to start your <b>MCA journey</b>? Reach out to us for{" "}
            <b>Free Counselling, Admission Queries, Feedback,</b> or <b>Suggestions.</b>
          </p>
        </motion.div>
            ...
            >
              <Card className="bg-white/90 backdrop-blur-xl border border-pink-100 shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 rounded-2xl">
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-sm font-medium text-gray-600">{contact.primary}</p>
      ...
      <section id="free-counselling" className="scroll-mt-24 py-14 sm:py-20 bg-gray-50">
        <div className="max-w-6xl mx-auto px-6 text-center">
          ...
          >
            At <b>ACME Academy</b>, we believe every student deserves personalized guidance. Whether you&apos;re confused about{" "}
            <b>which course to choose</b>, want to know <b>exam preparation strategies</b>, or need to share feedback/suggestions
            — our expert mentors are just a message away. <b>Fill out the form below</b> to book your **Free Counselling
            Session** today.
          </motion.p>
        </div>
      </section>
      ...
      <section className="py-20 bg-gradient-to-b from-white via-pink-50 to-red-50">
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-12 items-start">
          ...
          >
            ...
            <p className="text-gray-500 text-lg">
              Join <b>ACME Academy</b> — India&apos;s <b>#1 MCA Coaching Institute</b> trusted by toppers across Kanpur,
              Raipur, and all over India.
            </p>
          ...
          {centers.map((center, index) => (
            ...
            >
              <div className="p-4">
                ...
                <a href={`tel:${center.phone}`} className="text-pink-600 text-sm hover:underline">
                  {center.phone}
                </a>
              </div>
              <iframe
        ...
        <div className="mt-20 max-w-6xl mx-auto grid md:grid-cols-3 gap-8 text-center">
          {stats.map((s, i) => (
            <AnimatedStat key={i} icon={s.icon} value={s.value} label={s.label} delay={i * 0.1} />
          ))}
        </div>
      </section>
    </div>
