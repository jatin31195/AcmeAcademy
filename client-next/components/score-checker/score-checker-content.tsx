...
import { Flask_URL, BASE_URL } from "@/lib/config";

// Ported from client/src/pages/ScoreCheckerPage.jsx. The bundled NIMCET
// answer-key PDF was a Vite asset import (`import answerKeyPdfUrl from
// "@/assets/question_answer_ids_separated_1.pdf"`), which resolves to a
// build-time URL string — Next.js/Turbopack has no equivalent loader for
// arbitrary binary asset imports, so the file was copied to
// public/assets/question_answer_ids_separated_1.pdf and referenced by that
// public path string instead, which is fetched identically at runtime.
const answerKeyPdfUrl = "/assets/question_answer_ids_separated_1.pdf";
...
type CandidateInfo = { name?: string; app_no?: string; roll_no?: string };
type ScoreCardData = { score: number; correct: number; incorrect: number; unattempted: number; candidate?: CandidateInfo };
type QuestionRow = { q_no?: number; qid?: string; section?: string; chosen?: string; correct?: string; yours?: string; status: string; marks?: number };
type SectionStat = { score: number; correct: number; wrong: number; unattempted: number };
type NimcetResult = {
  total: number;
  totals?: { correct: number; wrong: number; unattempted: number; total_questions?: number };
  stats?: Record<string, SectionStat>;
  candidate?: CandidateInfo;
  results: QuestionRow[];
};
type CuetResult = { score: number; correct: number; incorrect: number; unattempted: number; results: QuestionRow[] };
...
// ── Scorecard download ────────────────────────────────────────────────────────
const downloadScoreCard = (result: ScoreCardData, examLabel: string, scoringNote = "+4 correct  ·  −1 incorrect  ·  0 unattempted") => {
  ...
  const drawContent = (logoImg: HTMLImageElement | null) => {
    ...
    stats.forEach((s, i) => {
      ...
      ctx.fillStyle = s.bg;
      ctx.beginPath();
      ctx.roundRect(x, 148, 150, 112, 12);
      ctx.fill();
    ...
    ctx.beginPath();
    ctx.roundRect(448, 334, 480, 6, 3);
    ctx.fill();
    const accGrad = ctx.createLinearGradient(448, 0, 928, 0);
    accGrad.addColorStop(0, "#9333ea");
    accGrad.addColorStop(1, "#ec4899");
    ctx.fillStyle = accGrad;
    ctx.beginPath();
    ctx.roundRect(448, 334, 480 * pct, 6, 3);
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.roundRect(52, 370, 896, 70, 10);
    ctx.fill();
...
// ── Upload zone ───────────────────────────────────────────────────────────────
const UploadZone = ({ label, icon, file, onFile }: { label: string; icon: string; file: File | null; onFile: (f: File) => void }) => {
  ...
  return (
    ...
    >
      ...
      {file ? <p className="text-xs text-green-600 font-medium mt-2 break-all">✓ {file.name}</p> : <p className="text-xs text-gray-400 mt-1">Drag & drop or click · PDF only</p>}
    </motion.div>
  );
};

// ── Main page ─────────────────────────────────────────────────────────────────
export function ScoreCheckerContent() {
  const auth = useAuth();
  ...
  const [result, setResult] = useState<NimcetResult | CuetResult | null>(null);
  ...
  const userName = String((user as { fullname?: string; name?: string } | undefined)?.fullname || (user as { name?: string } | undefined)?.name || "").trim();
  const userPhone = String((user as { phone?: string; whatsapp?: string } | undefined)?.phone || (user as { whatsapp?: string } | undefined)?.whatsapp || "")
    .replace(/\D/g, "")
    .slice(-10);
  ...
  const handleResponseFileChange = (file: File) => {
  ...
  const sendNimcetOtp = async () => {
    ...
    try {
      ...
      if (res.status === 429) {
        setOtpError(data.message || "Too many OTP requests. Please wait a few minutes.");
        return;
      }
      if (!res.ok) {
        setOtpError(data.message || data.Details || "Failed to send OTP.");
        return;
      }
  ...
  const verifyNimcetOtp = async () => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (res.status === 429) {
        setOtpError(data.message || "Too many attempts. Please wait a moment and try again.");
        return;
      }
      if (!res.ok) {
        setOtpError(data.message || data.Details || "Could not verify OTP. Please try again.");
        return;
      }
      if (data.Status === "Success") {
        setOtpVerified(true);
  ...
  const handleCheck = async () => {
    ...
    try {
      ...
      form.append("response_sheet", responseFile as File);
      form.append("answer_key", answerFile as File);
  ...
  const nimcetResult = exam === "NIMCET" ? (result as NimcetResult | null) : null;
  const cuetResult = exam === "CUET_PG" ? (result as CuetResult | null) : null;
  ...
  const downloadNimcetScoreCard = () => {
    if (!nimcetResult?.totals) return;
    downloadScoreCard(
      {
        score: nimcetResult.total,
        correct: nimcetResult.totals.correct,
        incorrect: nimcetResult.totals.wrong,
        unattempted: nimcetResult.totals.unattempted,
        candidate: nimcetResult.candidate,
      },
      "NIMCET",
  ...
  const downloadNimcetReport = async () => {
    ...
    try {
      ...
      const link = document.createElement("a");
      link.href = url;
      link.download = `ACME_Academy_NIMCET_Report_${nimcetResult?.candidate?.app_no || "Student"}.pdf`;
      link.click();
  ...
  const filtered: QuestionRow[] = result ? (filter === "All" ? result.results : result.results.filter((r) => r.status === filter)) : [];

  // CUET accuracy
  const pct = cuetResult ? Math.round((cuetResult.correct / (cuetResult.correct + cuetResult.incorrect + cuetResult.unattempted)) * 100) : 0;

  // NIMCET accuracy
  const nimcetPct = nimcetResult?.totals ? Math.round((nimcetResult.totals.correct / (nimcetResult.totals.total_questions || 1)) * 100) : 0;
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: -30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            <h1 className="text-[clamp(2.2rem,5vw,3.8rem)] font-extrabold text-white drop-shadow-2xl mb-4">
              <span className="bg-gradient-to-r from-pink-400 via-purple-300 to-indigo-300 text-transparent bg-clip-text">Score</span> <span className="text-white">Checker</span>
            </h1>
            ...
            <p className="font-medium text-sm sm:text-base text-white/60 max-w-lg mx-auto leading-relaxed mb-6">
              India&apos;s only MCA-focused score checker — built by ACME Academy experts who know these exams inside out. Upload your NTA PDFs and get your exact score,
              accuracy & a branded result card in seconds.
            </p>
            <div className="flex flex-wrap justify-center gap-3 mt-2">
              {["⚡ Instant Results", "🔒 100% Private", "📄 Downloadable Card", "🎯 CUET PG & NIMCET"].map((b) => (
                <span key={b} className="bg-white/10 backdrop-blur border border-white/20 text-white/80 text-xs font-semibold px-3 py-1.5 rounded-full">
                  {b}
                </span>
              ))}
            </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {exam === "NIMCET" && !otpVerified && !result && (
            <motion.div key="nimcet-otp" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }}>
              <div className="flex items-center gap-3 mb-8">
                <button onClick={reset} className="text-gray-400 hover:text-pink-600 text-sm transition-colors">
                  ← Change Exam
                </button>
              ...
              <div className="max-w-md mx-auto rounded-3xl bg-white/70 backdrop-blur-xl border border-white/30 shadow-xl p-8">
                <div className="text-center mb-6">
                  ...
                  <h2 className="text-xl font-extrabold text-gray-800 mb-1">{otpStep === "phone" ? "Verify Your Mobile Number" : "Enter OTP"}</h2>
                  <p className="text-xs text-gray-500">
                    {otpStep === "phone" ? "We'll send a one-time password to confirm it's you before you upload your response sheet." : `OTP sent to +91 ${otpPhone}`}
                  </p>
                </div>
                ...
                <AnimatePresence>
                  {otpError && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mt-5 bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-red-600 text-sm">
                      ❌ {otpError}
                    </motion.div>
                  )}
                </AnimatePresence>

                <p className="mt-6 pt-5 border-t border-gray-100 text-center text-[11px] text-gray-400">🔒 Your number is secure and never shared</p>
              </div>
            </motion.div>
          )}
            ...
            <motion.div key="nimcet-results" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <motion.button
                    whileHover={reportLoading ? {} : { scale: 1.05 }}
              ...
              {nimcetResult.candidate && (nimcetResult.candidate.name || nimcetResult.candidate.app_no) && (
                <div className="rounded-2xl bg-white/70 backdrop-blur-xl border border-white/30 shadow-sm px-6 py-4 mb-6 flex flex-wrap items-center gap-x-10 gap-y-3">
                  <div>
                    ...
                    <p className="text-sm font-bold text-gray-800">{nimcetResult.candidate.name || "—"}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-gray-400 font-bold tracking-wider uppercase">Application No</p>
                    <p className="text-sm font-bold text-gray-800">{nimcetResult.candidate.app_no || "—"}</p>
                  </div>
                  <Link href="/contact-acme-academy#free-counselling" className="sm:ml-auto w-full sm:w-auto">
                    <motion.button
              ...
              <div className="relative overflow-hidden rounded-3xl bg-white/70 backdrop-blur-xl border border-white/30 shadow-xl p-10 text-center mb-6">
                ...
                <p className="text-gray-400 text-sm">out of 1000 · {nimcetResult.totals?.total_questions || 120} questions</p>
              ...
              <div className="grid grid-cols-3 gap-4 mb-8">
                {[
                  { label: "✅ Correct", val: nimcetResult.totals?.correct, col: "text-green-600", bg: "bg-green-50  border-green-200" },
                  { label: "❌ Wrong", val: nimcetResult.totals?.wrong, col: "text-red-500", bg: "bg-red-50    border-red-200" },
                  { label: "⚪ Unattempted", val: nimcetResult.totals?.unattempted, col: "text-gray-500", bg: "bg-gray-50   border-gray-200" },
                ].map((s, i) => (
                  <motion.div
                    key={i}
                    initial={{ opacity: 0, y: 20 }}
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {NIMCET_SECTIONS.map((sec, i) => {
                    const s = nimcetResult.stats?.[sec.key] || ({} as Partial<SectionStat>);
                    return (
                      <motion.div
                        key={sec.key}
                        initial={{ opacity: 0, y: 20 }}
                      ...
                      >
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <div className="text-right">
                            ...
                            <p className={`text-2xl font-extrabold ${(s.score ?? 0) < 0 ? "text-red-500" : "text-pink-600"}`}>{s.score}</p>
                          </div>
                        </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
                  ...
                  <p className="text-xs text-gray-400">{nimcetResult.results.length} questions total</p>
                </div>
                <div className="flex gap-2 flex-wrap mb-5">
                  {["All", "Correct", "Incorrect", "Unattempted"].map((f) => {
                    const count = f === "All" ? nimcetResult.results.length : nimcetResult.results.filter((r) => r.status === f).length;
                    return (
                      <button
                        key={f}
                        onClick={() => setFilter(f)}
                    ...
                    <thead>
                      <tr className="border-b border-gray-100 bg-white/80">
                        {["#", "Section", "Chosen", "Correct", "Status", "Marks"].map((h) => (
                          <th key={h} className="px-4 py-3 text-left text-gray-400 font-bold tracking-wider uppercase">
                            {h}
                          </th>
                        ))}
                      </tr>
                    ...
                    <tbody>
                      {filtered.map((row) => (
                        <tr key={row.q_no} className="border-b border-gray-50 hover:bg-pink-50/30 transition-colors">
                          ...
                          <td className={`px-4 py-3 font-bold ${(row.marks ?? 0) > 0 ? "text-green-600" : (row.marks ?? 0) < 0 ? "text-red-500" : "text-gray-400"}`}>{row.marks}</td>
                        </tr>
                      ))}
                      {filtered.length === 0 && (
                        <tr>
          ...
          {exam === "CUET_PG" && !result && (
            <motion.div key="cuet-upload" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.4 }}>
              <div className="flex items-center gap-3 mb-8">
                <button onClick={reset} className="text-gray-400 hover:text-purple-600 text-sm transition-colors">
                  ← Change Exam
                </button>
                <span className="text-gray-200">|</span>
            ...
            <motion.div key="results" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
                ...
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.97 }}
                  onClick={() => downloadScoreCard(cuetResult, "CUET PG")}
              ...
              <div className="grid grid-cols-3 gap-4 mb-8">
                {[
                  { label: "✅ Correct", val: cuetResult.correct, col: "text-green-600", bg: "bg-green-50  border-green-200" },
                  { label: "❌ Incorrect", val: cuetResult.incorrect, col: "text-red-500", bg: "bg-red-50    border-red-200" },
                  { label: "⚪ Unattempted", val: cuetResult.unattempted, col: "text-gray-500", bg: "bg-gray-50   border-gray-200" },
                ].map((s, i) => (
                  <motion.div
                    key={i}
                    initial={{ opacity: 0, y: 20 }}
              ...
              <div>
                <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
                  <h2 className="text-base font-extrabold text-gray-800">📊 Question-wise Analysis</h2>
                  <p className="text-xs text-gray-400">{cuetResult.results.length} questions total</p>
                </div>
                <div className="flex gap-2 flex-wrap mb-5">
                  {["All", "Correct", "Incorrect", "Unattempted"].map((f) => {
                    const count = f === "All" ? cuetResult.results.length : cuetResult.results.filter((r) => r.status === f).length;
                    return (
                      <button
                        key={f}
                        onClick={() => setFilter(f)}
