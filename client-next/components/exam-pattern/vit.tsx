...
// Ported verbatim from client/src/components/ExamPattern/VIT.jsx.
const VIT = () => {
    ...
    <div className="space-y-8">
      ...
      <Tabs value={selectedTab} onValueChange={setSelectedTab} className="w-full">
          ...
          {["overview", "syllabus", "criteria", "institutes"].map((tab, i) => (
            <TabsTrigger
              ...
              className="
                ...
                "
            >
              {tab === "overview" ? "Overview" : tab === "syllabus" ? "Syllabus" : tab === "criteria" ? "Eligibility" : "Institutes"}
            </TabsTrigger>
          ))}
        </TabsList>
        ...
        <TabsContent value="syllabus">
          <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            {Object.entries(syllabus).map(([sub, topics], i) => (
              <Card key={i} className="bg-white/70 backdrop-blur-md border border-gray-200 rounded-2xl shadow-lg hover:shadow-xl transition-all">
                <CardHeader>
            ...
            {institutes.map((inst, i) => (
              <Card key={i} className="bg-white/70 backdrop-blur-md border border-gray-200 rounded-2xl shadow-lg hover:shadow-xl transition-all">
                ...
                <CardContent className="space-y-3 text-sm text-gray-700">
                  {inst.contact && (
                    <p>
                      <strong>Contact:</strong> {inst.contact}
                    </p>
                  )}
                  {inst.email && (
                    <p>
                  ...
                  {inst.program && (
                    <p>
                      <strong>Program:</strong> {inst.program}
                    </p>
                  )}
                  {inst.duration && (
                    <p>
                      <strong>Duration:</strong> {inst.duration}
                    </p>
                  )}
                </CardContent>
              </Card>
