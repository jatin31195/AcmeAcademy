...
// Ported verbatim from client/src/components/Test/SolutionView.jsx.
// Note: the original also accepted `sections`, `currentSectionIndex`, and
// `setCurrentSectionIndex` props from TestResult.jsx, but never read them —
// it recomputes its own `sections` from `results` and keeps its own local
// section/question index state. That dead prop-passing is not reproduced
// here since only `results` and `onExit` were ever actually used.
type ResultQuestion = {
  questionId?: string;
  questionText?: string;
  options: string[];
...
const renderWithMath = (text?: string | null) => {
...
const SolutionView = ({ results, onExit }: { results: TestResults; onExit: () => void }) => {
  const auth = useAuth();
  ...
  if (results.sections && results.sections.length > 0) {
    sections = results.sections.map((sec) => ({
      ...sec,
      questions: sec.questions && sec.questions.length > 0 ? sec.questions : results.questions || [],
    }));
  } else {
    sections = [
      {
  ...
  const getResultClass = (q: ResultQuestion) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      <div className="border-b px-4 py-3 flex items-center justify-between flex-wrap">
        ...
        <div className="text-sm text-gray-600 mt-2 md:mt-0">⏱️ Total Time: {formatTime(results.totalTimeTaken || 0)}</div>
      </div>

      {sections.length > 1 && (
        <div className="border-b flex items-center justify-between px-4 py-2 bg-cyan-500 text-white">
          <ChevronLeft className={`h-5 w-5 cursor-pointer ${currentSectionIndex === 0 ? "opacity-50 cursor-not-allowed" : ""}`} onClick={prevSection} />
      ...
      <div className="flex flex-1 md:flex-row flex-col overflow-hidden gap-4">
        <div className="flex-1 p-4 md:p-6 overflow-auto">
          <div className="mb-4 flex items-center justify-between flex-wrap">
            ...
            <span className="text-sm text-gray-500">⏱️ Time: {formatTime(currentQ.timeTaken || 0)}</span>
          </div>

          <div className="bg-gray-50 p-4 md:p-6 rounded-lg mb-4 text-gray-800 leading-relaxed">{renderWithMath(currentQ.questionText)}</div>
              ...
              return (
                <div
                  ...
                  className={`flex items-start gap-3 p-3 rounded border ${
                    isCorrectOption ? "bg-green-100 border-green-500" : isUserAnswer ? "bg-red-100 border-red-500" : "bg-white border-gray-200"
                  }`}
                >
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            ...
            <p className="text-sm text-gray-700 mb-1">✅ Correct Answer: {renderWithMath(currentQ.correctAnswer)}</p>
        ...
        <div className="w-full md:w-80 border-l bg-gray-50 p-4 md:p-6 overflow-auto flex-shrink-0">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-full bg-cyan-500 text-white flex items-center justify-center font-bold">
              {(user as { username?: string } | null)?.username?.[0]?.toUpperCase() || "U"}
            </div>
            <span className="font-semibold text-sm">{(user as { username?: string } | null)?.username || "Student"}</span>
          </div>
          ...
          <div className="grid grid-cols-5 gap-2 mb-6">
            {questions.map((q, i) => (
              <button
                ...
                className={`w-10 h-10 rounded flex items-center justify-center text-sm font-semibold transition-all ${getResultClass(q)} ${
                  currentQuestionIndex === i ? "ring-2 ring-cyan-500 ring-offset-2" : ""
                }`}
              >
                {i + 1}
              </button>
