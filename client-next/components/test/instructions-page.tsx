...
// Ported verbatim from client/src/components/Test/InstructionsPage.jsx.
type TestSection = { _id?: string; title: string; numQuestions?: number; marksPerQuestion?: number };
type TestData = {
  title?: string;
  examCode?: string;
...
const InstructionsPage = ({
  ...
  setInstructionsRead: (v: boolean) => void;
  onStart: () => void;
  username?: string;
}) => {
  ...
  if (!test) return <div className="p-10 text-center">Loading test info...</div>;
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex flex-1">
            ...
            {step === 1 && (
              <>
                ...
                <div className="mt-8 flex items-start space-x-3 bg-amber-50 p-4 rounded-lg border-2 border-amber-200 max-w-xl">
                  <Checkbox id="instructions" checked={instructionsRead} onCheckedChange={(checked) => setInstructionsRead(Boolean(checked))} className="mt-1" />
            ...
            {step === 2 && (
              <>
                ...
                {test.sections?.map((section, idx) => (
                  <div key={section._id || idx} className="border rounded-lg p-6 mb-8">
                    <div className="flex items-start gap-4">
                      <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center text-gray-600 font-semibold flex-shrink-0">{idx + 1}</div>
                      <div>
                ...
                <div className="flex items-start gap-3 p-4 bg-gray-50 rounded-lg border">
                  <Checkbox checked={agreedToTerms} onCheckedChange={(checked) => setAgreedToTerms(Boolean(checked))} className="mt-1" />
                  <label className="text-sm text-gray-700 cursor-pointer" onClick={() => setAgreedToTerms(!agreedToTerms)}>
                    ...
                    and/or disciplinary action, which may include a ban from future tests.
                  </label>
                </div>
              </>
            )}
        ...
        <div className="w-80 border-l bg-gray-50 p-6">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-full bg-red-500 text-white flex items-center justify-center font-bold text-lg">{username?.slice(0, 2).toUpperCase()}</div>
      ...
      <div className="border-t px-8 py-4 flex justify-between">
        {step === 2 ? (
          <Button variant="outline" onClick={() => setStep(1)} className="px-8 border-cyan-500 text-cyan-500 hover:bg-cyan-50">
        ...
        {step === 1 ? (
          <Button onClick={() => setStep(2)} disabled={!instructionsRead} className="bg-cyan-500 hover:bg-cyan-600 text-white px-8">
            Next <ChevronRight className="ml-2 h-4 w-4" />
          </Button>
        ) : (
          <Button onClick={handleStart} disabled={!agreedToTerms} className="bg-green-500 hover:bg-green-600 text-white px-8">
