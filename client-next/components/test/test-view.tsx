...
// Ported verbatim from client/src/components/Test/TestView.jsx.
type Question = { _id: string; question?: string; text?: string; options: string[] };
type Section = { title: string; numQuestions: number };
type TestData = { title: string; sections: Section[]; questions: Question[] };
...
const TestView = ({
  ...
  setCurrentQuestion: (updater: number | ((q: number) => number)) => void;
  answers: Record<string, string>;
  handleAnswer: (questionId: string, option: string) => void;
  handleMarkForReviewAndNext: () => void;
  handleSaveAndNext: () => void;
  handleSubmit: () => void;
  getQuestionStatusColor: (id: string) => string;
  statusCounts: { notVisited: number; notAnswered: number; answered: number; marked: number; answeredAndMarked: number };
  formatTime: (s: number) => string;
  timeLeft: number;
  sectionTimeLeft: number;
  currentSection: string;
}) => {
  const sectionIndex = testData.sections.findIndex((s) => s.title === currentSection);
  ...
  const safeQuestionIndex = Math.min(currentQuestion, sectionQuestions.length - 1);

  const currentQ = sectionQuestions[safeQuestionIndex];

  if (!currentQ) {
    return <div className="min-h-screen flex items-center justify-center text-gray-500">Loading section...</div>;
  }

  return (
    <div className="min-h-screen bg-white flex flex-col">
      <div className="border-b px-4 py-3 flex items-center justify-between flex-wrap">
        <h1 className="text-base font-medium">{testData.title}</h1>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="text-sm font-medium">
            Time Left: <span className={timeLeft < 300 ? "text-red-500" : ""}>{formatTime(timeLeft)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="border-b flex items-center px-4 py-2 bg-cyan-500 text-white">
        ...
        <div className="flex-1 flex flex-col items-center leading-tight">
          ...
          <span className={`text-xs ${sectionTimeLeft < 60 ? "text-red-200" : "text-white/90"}`}>Time Left: {formatTime(sectionTimeLeft)}</span>
        </div>
      ...
      <div className="flex flex-1 flex-col md:flex-row overflow-hidden gap-4">
        <div className="flex-1 p-4 md:p-6 overflow-auto">
          <div className="mb-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm">
                Marking: <span className="text-green-600 font-semibold">+4</span> <span className="text-red-600 font-semibold">-1</span>
              </span>

              <Button size="sm" onClick={handleMarkForReviewAndNext} className="border-cyan-500 text-cyan-500" variant="outline">
                👁️ Mark for Review
              </Button>
          ...
          <div className="bg-gray-50 p-4 md:p-6 rounded-lg mb-4">{renderWithMath(currentQ.question || currentQ.text)}</div>

          <div className="space-y-2">
            {currentQ.options.map((option, index) => (
              <label
                key={index}
                className={`flex gap-3 p-3 rounded cursor-pointer border ${answers[currentQ._id] === option ? "bg-green-100 border-green-500" : "hover:bg-gray-50"}`}
              >
                <input type="radio" checked={answers[currentQ._id] === option} onChange={() => handleAnswer(currentQ._id, option)} />
        ...
        <div className="w-full md:w-80 border-l bg-gray-50 p-4 md:p-6 overflow-auto">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-full bg-red-500 text-white flex items-center justify-center font-bold">{user?.username?.[0]?.toUpperCase() || "U"}</div>
          ...
          <div className="grid grid-cols-5 gap-2">
            {sectionQuestions.map((q, i) => (
              <button key={q._id} onClick={() => setCurrentQuestion(i)} className={`w-10 h-10 rounded font-semibold ${getQuestionStatusColor(q._id)}`}>
                {i + 1}
              </button>
            ))}
          </div>
