...
import { BookOpen, Clock, Laptop, Video, type LucideIcon } from "lucide-react";
import { BASE_URL } from "@/lib/config";

// Ported from client/src/components/home/CoursesSection.jsx. Client-fetched
// (useEffect + fetch), so its JSON-LD is only ever available after the
// client-side fetch resolves — same limitation the original had with
// react-helmet-async (Helmet also can't inject JSON-LD before the fetch
// completes). A plain <script> tag rendered here reproduces that identical
// behavior; it isn't part of the initial server-rendered HTML either way.
...
const CoursesSection = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const res = await fetch(`${BASE_URL}/api/get-course/home-courses`);
  ...
  return (
    <section className="py-24 bg-gradient-to-b from-blue-50/40 to-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        {!loading && courses.length === 0 && <p className="text-center text-muted-foreground">No courses available</p>}

        <div className="grid md:grid-cols-3 gap-8">
          {courses.map((course, index) => {
            const Icon = (course.icon && ICON_MAP[course.icon]) || BookOpen;
                ...
                <Card className="relative glass hover-glow transition-all duration-300 hover:scale-105 hover:shadow-xl border border-gray-200/60 rounded-2xl h-full flex flex-col justify-between">
                  ...
                  <div>
                    ...
                    <CardContent className="relative z-10">
                      <p className="text-muted-foreground mb-3 text-sm leading-relaxed line-clamp-5">{course.description}</p>
                    </CardContent>
                  </div>

                  <div className="relative z-10 p-4 flex justify-between text-xs font-medium">
                    {course.duration && (
                      <span className="bg-primary/10 px-3 py-1 rounded-full text-primary">⏳ {course.duration}</span>
                    )}
                    {course.mode && <span className="bg-primary/10 px-3 py-1 rounded-full text-primary">💻 {course.mode}</span>}
                  </div>
                </Card>
              </motion.a>
            );
