...
// ── Scorecard download with real logo ────────────────────────────────────────
const downloadScoreCard = (result, examLabel) => {
  ...
  // White/light background matching site
  ...
  const drawContent = (logoImg) => {
    ...
    stats.forEach((s, i) => {
      const x = 448 + i * 166;
      ctx.fillStyle = s.bg;
      ctx.beginPath(); ctx.roundRect(x, 148, 150, 112, 12); ctx.fill();
    ...
    ctx.beginPath(); ctx.roundRect(448, 334, 480, 6, 3); ctx.fill();
    ...
    ctx.beginPath(); ctx.roundRect(448, 334, 480 * pct, 6, 3); ctx.fill();

    // Bottom divider
    ctx.strokeStyle = "rgba(124,58,237,0.1)";
    ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(52, 396); ctx.lineTo(948, 396); ctx.stroke();

    // Footer
    ctx.font = "500 11px Georgia, serif";
    ctx.fillStyle = "#9ca3af";
    ctx.fillText("Generated by ACME Academy · acmeacademy.in", 52, 424);
    ctx.fillText(new Date().toLocaleString("en-IN", { dateStyle: "long", timeStyle: "short" }), 52, 444);
    ctx.font = "500 11px Georgia, serif";
    ctx.fillStyle = "#7c3aed";
    ctx.fillText("🔗 acmeacademy.in/score-checker", 52, 464);
    ctx.textAlign = "right";
    ctx.font = "italic 11px Georgia, serif";
    ctx.fillStyle = "#c4b5fd";
    ctx.fillText("India's Most Trusted MCA Entrance Academy", 948, 424);
    ctx.textAlign = "left";
    ...
    ctx.fillStyle = "#7c3aed";
    ctx.translate(500, 500); ctx.rotate(-0.15);
...
const ScoreCheckerPage = () => {
  ...
  const reset = () => {
    setExam(null); setResponseFile(null); setAnswerFile(null);
  ...
  return (
    <>
      ...
      {/* ── Outer wrapper — matches site's light gradient bg ── */}
      <div className="min-h-screen bg-gradient-to-b from-gray-50 via-gray-100 to-gray-200">

        {/* Hero section — exact pattern from About page */}
        <section className="relative py-16 sm:py-26 text-center overflow-hidden hero-gradient">
            ...
            >
              ...
              <div className="flex flex-wrap justify-center gap-3 mt-2">
                {[
                  ...
                  <span key={b}
                    className="bg-white/10 backdrop-blur border border-white/20 text-white/80 text-xs font-semibold px-3 py-1.5 rounded-full">
                    {b}
                  </span>
                ))}
              </div>
          ...
          <div className="absolute bottom-0 left-0 w-full overflow-hidden leading-[0]">
            <svg className="relative block w-full h-16" xmlns="http://www.w3.org/2000/svg"
              preserveAspectRatio="none" viewBox="0 0 1200 120">
              <path d="M985.66 92.83C906.67 72 823.78 48.49 743.84 26.94 661.18 4.8 578.56-5.45 497.2 1.79 423.15 8.3 349.38 28.74 278.07 51.84 183.09 83.72 90.6 121.65 0 120v20h1200v-20c-80.3-1.6-160.39-26.5-214.34-47.17z"
                fill="white" />
            </svg>
          </div>
        </section>
        ...
        <div className="relative max-w-4xl mx-auto px-4 py-12 pb-24">
          ...
          <AnimatePresence mode="wait">
            ...
            {!exam && (
              ...
              >
                <p className="text-center text-gray-400 text-xs font-bold tracking-[0.2em] uppercase mb-8">
                  Select Your Exam
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-2xl mx-auto">
                  {EXAMS.map((e, i) => (
                    ...
                    >
                      ...
                      {/* badge */}
                      <div className={`absolute top-4 right-4 text-[10px] font-black px-2.5 py-0.5 rounded-full border ${e.badgeCls}`}>
                        {e.badge}
                      </div>
            ...
            {exam === "NIMCET" && (
              ...
              >
                ...
                <p className="text-gray-500 leading-relaxed mb-8">
                  We are working at high speed to deliver you the most accurate NIMCET score checker.
                  Our team is calibrating the exact marking scheme — hold tight!
                </p>

                <div className="bg-white/70 backdrop-blur-xl border border-white/30 rounded-2xl shadow-lg p-6 mb-8 text-left">
                  <p className="text-[10px] text-gray-400 font-bold tracking-[0.2em] uppercase mb-4">What's coming</p>
                  {[
                    "NIMCET response sheet PDF parsing",
                    "Subject-wise breakdown (Maths, Reasoning, CS, English)",
                    "Percentile estimator against past cutoffs",
                    "Downloadable rank card with ACME branding",
                  ].map((f, i) => (
                ...
                <button onClick={reset}
                  className="bg-white border border-gray-200 hover:border-purple-400 text-gray-600 hover:text-purple-600 px-8 py-3 rounded-full text-sm font-semibold transition-all shadow-sm">
                  ← Choose Another Exam
                </button>
              </motion.div>
            )}
                  ...
                  <div className="flex gap-2 flex-wrap mb-5">
                    {["All", "Correct", "Incorrect", "Unattempted"].map(f => {
                      const count = f === "All"
                        ...
                        : result.results.filter(r => r.status === f).length;
                      return (
